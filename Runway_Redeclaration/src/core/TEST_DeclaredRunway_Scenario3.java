...
public class TEST_DeclaredRunway_Scenario3 {
	...
	public void testAddingObstacleWithGoodID_On_SMALL() throws UnusableRunwayException{
		Obstacle obj = new Obstacle("Scenario 1 Obstacle", 0, 15);
		double dist = 150;
		try {
			...
			//--[ small angled ]------------
			assertEquals("NEW Small TORA",2903,air.getSmallAngledRunway().getTORA(),0);
			assertEquals("NEW Small ASDA",2903,air.getSmallAngledRunway().getASDA(),0);
			assertEquals("NEW Small TODA",2903,air.getSmallAngledRunway().getTODA(),0);
			assertEquals("NEW Small LDA",2393,air.getSmallAngledRunway().getLDA(),0);
			...
			//--[ Large angled ]------------
			assertEquals("NEW Large TORA",2393,air.getLargeAngledRunway().getTORA(),0);
			assertEquals("NEW Large ASDA",2393,air.getLargeAngledRunway().getASDA(),0);
			assertEquals("NEW Large TODA",2393,air.getLargeAngledRunway().getTODA(),0);
			assertEquals("NEW Large LDA",2903,air.getLargeAngledRunway().getLDA(),0);
